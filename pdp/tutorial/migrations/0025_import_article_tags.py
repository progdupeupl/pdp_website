...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for a in orm['article.Article'].objects.all():
            tuto = orm.Tutorial.objects.get(title=a.title)
            ct = orm['contenttypes.contenttype'].objects.get(
                app_label='article',
                model='article'
            )
            tags_db = orm['taggit.taggeditem'].objects.filter(
                content_type=ct,
                object_id=a.id
            )
            tags = [t.tag.name for t in tags_db]
            for tag_name in tags:
                ct = orm['contenttypes.contenttype'].objects.get(
                    app_label='tutorial',
                    model='tutorial'
                )
                tag = fake_get_or_create(
                    orm['taggit.tag'],
                    name=tag_name
                )
                tagged_item = fake_get_or_create(
    ...
    complete_apps = ['article', 'taggit', 'tutorial']
    symmetrical = True
